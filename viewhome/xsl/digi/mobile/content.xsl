	...
	<xsl:template match="/">
		<html lang="zh_cn">
			<head>
				...
				<script src="/view/js/aa.js"></script>
					...
					<div data-role="content" align="center">
						<script>
							...
							function post(value, flowid, confirmflag, confirmstr){	
								...
								var url = "/view/oamobile/request/Produce/ProInd.nsf/THFlowBackTraceAgent?openagent&login";
						...
						<div class="ui-grid-b">
							<div class="ui-block-a" style="padding-bottom:5px;" align="center">
								...
								<xsl:if test="//.[@class='DB_SET_TD' and not(contains(@style, 'none'))]/a[contains(@href, 'submit')]">
							...
							<div class="ui-block-b" style="padding-bottom:5px;" align="center">
								<xsl:if test="//.[@class='DB_SET_TD' and not(contains(@style, 'none'))]/a[contains(@href, 'reject')]">
							...
							<div class="ui-block-c" style="padding-bottom:5px;" align="center">
								<xsl:if test="//.[@class='DB_SET_TD' and not(contains(@style, 'none'))]/a[contains(@href, 'moreoption')]">
								<a href="#popupBasic" data-rel="popup" data-role="button" data-mini='true' data-theme="f">高 级</a>
								</xsl:if>
								<div data-role="popup" id="popupBasic">
								<ul data-role="listview" data-inset="true" data-theme="c">
									<li data-role="list-divider"></li>
									<xsl:if test="//.[@class='DB_SET_TD' and not(contains(@style, 'none'))]/a[contains(@onclick, 'append')]">
						...
						<div style="display:none">
							<table>
								<tr>  
									<td>
										<xsl:variable name="Value"><xsl:value-of select="//div"/></xsl:variable>
										<xsl:variable name="Value2">
											<xsl:value-of select="substring-after($Value,'应用数据库路径：')"/>
										</xsl:variable>
										<textarea name="ISCW" id="ISCW">
										<xsl:value-of select="substring-before($Value2,'.nsf')"/>
										</textarea> 
									</td>
								</tr>
								<tr>  
									<td>			
										<textarea name="ISCWZC" id="ISCWZC">
										<xsl:value-of select="//fieldentries/fieldentry[1]"/>
										</textarea> 
									</td>
								</tr>
								<tr>  
									<td>			
										<textarea name="isUseFlowWS" id="isUseFlowWS">
											<xsl:value-of select="//input[@name='isUseFlowWS']/@value"/>
										</textarea> 
									</td>
								</tr>
							</table>							
	...
	<xsl:template name="file">
		...
		<xsl:choose>
			<xsl:when test="contains($info, ';')">
				<a href="javascript:void(0)" onclick="viewfile('/view/oamobile/file/Produce/DigiFlowMobile.nsf/0/{//input[@name='AttachDocUnid']/@value}/$file/{substring-before($info, '(')}');"  data-role="button"><xsl:value-of select="substring-before($info, '(')"/></a>
				<xsl:call-template name="file">
			...
			<xsl:otherwise>
				<a href="javascript:void(0)" onclick="viewfile('/view/oamobile/file/Produce/DigiFlowMobile.nsf/0/{//input[@name='AttachDocUnid']/@value}/$file/{$info}');"  data-role="button"><xsl:value-of select="$info"/></a>
			</xsl:otherwise>
		</xsl:choose>
		</li>
	</xsl:template>
	
	<!-- 处理 基本信息 <xsl:if test="not(contains($sub, 'Tra'))">-->
